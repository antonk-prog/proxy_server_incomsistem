...
Parser::Parser(AsyncLogger * logger)
    : logger(logger),
      prepared_statements() {
    if (logger == nullptr) {
...
bool Parser::parseClientMessage(const char * data, size_t len) {
    ...
    while (offset + 5 <= len) {
        ...
        uint32_t msg_len = ntohl(*reinterpret_cast<const uint32_t*>(&data[offset + 1]));

        if (offset + 1 + msg_len > len) break;  

        const char* msg = data + offset + 5;
        size_t msg_size = msg_len - 4;

        switch (type) {
            case 'Q': logged |= parseQ(msg, msg_size); break;
            case 'P': logged |= parseP(msg, msg_size); break;
            case 'E': logged |= parseE(msg, msg_size); break;
            case 'B': logged |= parseB(msg, msg_size); break;
            case 'S': logged |= parseS(msg, msg_size); break; 
            case 'X': logged |= parseX(msg, msg_size); break;
            case 'C': logged |= parseC(msg, msg_size); break;
            case 'D': logged |= parseD(msg, msg_size); break;
            case 'H': logged |= parseH(msg, msg_size); break;
            case 'F': logged |= parseF(msg, msg_size); break;
            default:
                break;
        }
...
void Parser::logQuery(const std::string& query) {
    logger->log(query);
}
...
bool Parser::parseP(const char* data, size_t len) {
    ...
    if (stmt_len + 1 + query_len > len) return false;
...
bool Parser::parseE(const char* data, size_t len) {
    ...
    if (stmt_it != portal_to_statement.end()) {
        ...
        if (prep_it != prepared_statements.end()) {
            logQuery("[EXECUTE] " + portal + " → " + stmt_name + ": " + prep_it->second);
        } else {
            logQuery("[EXECUTE] " + portal + " → unknown statement: '" + stmt_name + "'");
        }
    } else {
...
bool Parser::parseB(const char* data, size_t len) {
    ...
    if (portal_len + 1 + stmt_len + 1 < len) {
        params = parseParameters(params_ptr, len - (portal_len + 1 + stmt_len + 1));
    }

    logQuery("[BIND] " + portal + " → " + stmt + (params.empty() ? "" : " (" + params + ")"));
    return true;
}
...
std::string Parser::parseParameters(const char* data, size_t len) {
    ...
    for (uint16_t i = 0; i < num_params && (ptr - data) < static_cast<ptrdiff_t>(len); ++i) {
        if (i > 0) oss << ", ";
        if (*ptr == '\x01') { 
            oss << "TEXT:";
            ptr++;
            uint32_t param_len = ntohl(*reinterpret_cast<const uint32_t*>(ptr));
            ptr += 4;
            if ((ptr - data) + param_len > static_cast<ptrdiff_t>(len)) break;
...
bool Parser::parseC(const char* data, size_t len) {
    ...
    switch (close_type) {
        case 'S': logQuery("[CLOSE STATEMENT] " + name); break;
        case 'P': logQuery("[CLOSE PORTAL] " + name); break;
        default: break;
    }
    return true;
}
...
bool Parser::parseD(const char* data, size_t len) {
    ...
    switch (desc_type) {
        case 'S': logQuery("[DESCRIBE STATEMENT] " + name); break;
        case 'P': logQuery("[DESCRIBE PORTAL] " + name); break;
        default: break;
    }
    return true;
}
